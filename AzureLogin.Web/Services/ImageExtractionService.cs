...
public class ImageExtractionService : IImageExtractionService
{
    ...
    public async Task<ImageExtractionResult> ExtractWithAzureVisionAsync(byte[] imageBytes, string mimeType)
    {
        try
        {
            ...
            {
                _logger?.LogWarning("Azure Vision service not available, falling back to AI detection");
                return await ExtractImagesAsync(imageBytes, mimeType);
            }

            _logger?.LogInformation("Starting Azure Vision extraction...");

            var visionResult = await _azureVisionService.AnalyzeImageAsync(imageBytes);
            ...
            {
                // Skip text-only regions
                if (IsTextOnlyRegion(region, textRegions)) continue;
                ...
                var padding = 4;
                var paddedBox = new BoundingBox
                {
                    X = Math.Max(0, boundingBox.X - padding),
                    Y = Math.Max(0, boundingBox.Y - padding),
                    Width = Math.Min(boundingBox.Width + padding * 2, visionResult.ImageWidth - Math.Max(0, boundingBox.X - padding)),
                    Height = Math.Min(boundingBox.Height + padding * 2, visionResult.ImageHeight - Math.Max(0, boundingBox.Y - padding)),
                    NormalizedX = boundingBox.NormalizedX, NormalizedY = boundingBox.NormalizedY,
                    NormalizedWidth = boundingBox.NormalizedWidth, NormalizedHeight = boundingBox.NormalizedHeight
                };

                var cropResult = await CropRegionAsync(imageBytes, mimeType, paddedBox);
                ...
                if (cropResult.Success && cropResult.ImageData != null)
                {
                    ...
                    extractedImages.Add(new ExtractedImage
                    {
                        ...
                        ImageType = imageType, BoundingBox = paddedBox,
            ...
            return new ImageExtractionResult
            {
                ...
                AnalysisSummary = $"Azure Vision detected {extractedImages.Count} regions with pixel-accurate bounds"
            };
        }
        catch (Exception ex)
        {
            ...
            return new ImageExtractionResult
            {
                Success = false, ErrorMessage = $"Azure Vision extraction failed: {ex.Message}",
                Images = new List<ExtractedImage>(), TotalImagesFound = 0
            };
        }
    }
